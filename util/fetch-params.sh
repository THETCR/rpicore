...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/RPICOINParams"
    else
        PARAMS_DIR="$HOME/.rpicoin-params"
    fi
fi
...
Failed to fetch the RPICOIN zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
RPICOIN - fetch-params.sh

This script will fetch the RPICOIN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common RPICOIN zkSNARK parameters. Note that it is
