...
bool RecalculateRPISupply(int nHeightStart, bool fSkipZrpi)
{
    ...
    uiInterface.ShowProgress(_("Recalculating RPI supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating RPI supply..."), percent);
        }
        ...
        // Rewrite zrpi supply too
        if (!fSkipZrpi && consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC)) {
