...
public:
    ...
    bool EraseMSDisabledAddresses(const std::vector<std::string>& vDisabledAddresses);
    ...
    bool WriteZRPISeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZRPISeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZRPISeed_deprecated(uint256& seed);
    bool EraseZRPISeed();
    bool EraseZRPISeed_deprecated();

    bool WriteZRPICount(const uint32_t& nCount);
    bool ReadZRPICount(uint32_t& nCount);
