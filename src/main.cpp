...
/** Fees smaller than this (in urpi) are considered zero fee (for relaying, mining and transaction creation)
                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy zRPI mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zRPI mint output in tx %s",
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    //Track zRPI money supply
    if (!UpdateZRPISupplyDisconnect(block, pindex)) {
        error("%s: Failed to calculate new zRPI supply", __func__);
        return DISCONNECT_FAILED;
    }
...
{
    util::ThreadRename("rpicoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // Update zRPI money supply map
    if (!UpdateZRPISupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zRPI supply for block=%s height=%d", __func__,
                                    block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    }

    // A one-time event where the zRPI supply was off (due to serial duplication off-chain on main net)
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculateRPISupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Rpicoin Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nRpicoinBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nRpicoinBadBlockBits)) {
            // accept RPICOIN block minted with incorrect proof of work threshold
            return true;
        }
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if(hasZRPIInputs && in.IsZerocoinSpend())
            ...
            if(hasZRPIInputs) {
                for (const CTxIn& zRpiInput : zRPIInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zRpiInput);
