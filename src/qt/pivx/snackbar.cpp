...
#include "qt/rpicoin/forms/ui_snackbar.h"
#include "qt/rpicoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(RPICOINGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &RPICOINGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
