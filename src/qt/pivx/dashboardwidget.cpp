...
#include "qt/rpicoin/forms/ui_dashboardwidget.h"
#include "qt/rpicoin/sendconfirmdialog.h"
#include "qt/rpicoin/txrow.h"
...
DashboardWidget::DashboardWidget(RPICOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareRpi, "square-chart-rpi");
    setCssProperty(ui->labelSquarezRpi, "square-chart-zrpi");
    setCssProperty(ui->labelRpi, "text-chart-rpi");
    setCssProperty(ui->labelZrpi, "text-chart-zrpi");
    ...
    setCssProperty(ui->labelAmountRpi, "text-stake-rpi-disable");
    setCssProperty(ui->labelAmountZrpi, "text-stake-zrpi-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &RPICOINGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &RPICOINGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &RPICOINGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("RPI")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isRpi = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZRPI;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair RPI, zRPI
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            rpi = (pair.first != 0) ? pair.first / 100000000 : 0;
            zrpi = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalRpi += pair.first;
            chartData->totalZrpi += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesRpi.append(rpi);
        chartData->valueszRpi.append(zrpi);

        int max = std::max(rpi, zrpi);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesRpi);
    set1->append(chartData->valueszRpi);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalRpi > 0 || chartData->totalZrpi > 0) {
        setCssProperty(ui->labelAmountRpi, "text-stake-rpi");
        setCssProperty(ui->labelAmountZrpi, "text-stake-zrpi");
    } else {
        setCssProperty(ui->labelAmountRpi, "text-stake-rpi-disable");
        setCssProperty(ui->labelAmountZrpi, "text-stake-zrpi-disable");
    }
    forceUpdateStyle({ui->labelAmountRpi, ui->labelAmountZrpi});
    ui->labelAmountRpi->setText(GUIUtil::formatBalance(chartData->totalRpi, nDisplayUnit));
    ui->labelAmountZrpi->setText(GUIUtil::formatBalance(chartData->totalZrpi, nDisplayUnit, true));
