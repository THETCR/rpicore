...
void CheckOffsetDisconnectedPeers(const CNetAddr& ip)
{
    ...
    if (setOffsetDisconnectedPeers.size() >= MAX_TIMEOFFSET_DISCONNECTIONS) {
        ...
        std::string strWarn2 = _("Please check that your computer's date and time are correct! If your clock is wrong RPICOIN Core will not work properly.");
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        std::string strDesc = "RPICOIN " + FormatFullVersion();
...
bool CConnman::BindListenPort(const CService& addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. RPICOIN Core is probably already running."), addrBind.ToString());
        else
