...
std::string ReindexZerocoinDB()
{
    ...
    while (pindex) {
        ...
        // update supply
        UpdateZRPISupplyConnect(block, pindex, true);
...
{
    CDataStream serializedCoinSpend = ZRPIModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZRPISupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    ...
    //Add mints to zRPI supply (mints are forever disabled after last checkpoint)
...
bool UpdateZRPISupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
    ...
    // Re-add spends to zRPI supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from zRPI supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);
