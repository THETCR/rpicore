...
def setup_repos():
    if not os.path.isdir('gitian.sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/rpicoin/gitian.sigs.git'])
    if not os.path.isdir('detached.sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/rpicoin/detached.sigs.git'])
    if not os.path.isdir('gitian-builder'):
        subprocess.check_call(['git', 'clone', 'https://github.com/devrandom/gitian-builder.git'])
    if not os.path.isdir('rpicore'):
        subprocess.check_call(['git', 'clone', 'https://github.com/rpicoin/rpicore.git'])
...
def build():
    ...
    os.makedirs('rpicoin-binaries/' + args.version, exist_ok=True)
    ...
    subprocess.check_call(['make', '-C', '../rpicore/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def sign():
    ...
    if args.windows:
        ...
        subprocess.check_call('cp inputs/rpicoin-' + args.version + '-win-unsigned.tar.gz inputs/rpicoin-win-unsigned.tar.gz', shell=True)
        subprocess.check_call(['bin/gbuild', '--skip-image', '--upgrade', '--commit', 'signature='+args.commit, '../rpicore/contrib/gitian-descriptors/gitian-win-signer.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-win-signed', '--destination', '../gitian.sigs/', '../rpicore/contrib/gitian-descriptors/gitian-win-signer.yml'])
        subprocess.check_call('mv build/out/rpicoin-*win64-setup.exe ../rpicoin-binaries/'+args.version, shell=True)
        subprocess.check_call('mv build/out/rpicoin-*win32-setup.exe ../rpicoin-binaries/'+args.version, shell=True)

    if args.macos:
        print('\nSigning ' + args.version + ' MacOS')
        subprocess.check_call('cp inputs/rpicoin-' + args.version + '-osx-unsigned.tar.gz inputs/rpicoin-osx-unsigned.tar.gz', shell=True)
        subprocess.check_call(['bin/gbuild', '--skip-image', '--upgrade', '--commit', 'signature='+args.commit, '../rpicore/contrib/gitian-descriptors/gitian-osx-signer.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-osx-signed', '--destination', '../gitian.sigs/', '../rpicore/contrib/gitian-descriptors/gitian-osx-signer.yml'])
        subprocess.check_call('mv build/out/rpicoin-osx-signed.dmg ../rpicoin-binaries/'+args.version+'/rpicoin-'+args.version+'-osx.dmg', shell=True)
    ...
    if args.commit_files:
        os.chdir('gitian.sigs')
        commit = False
        if os.path.isfile(args.version+'-win-signed/'+args.signer+'/rpicoin-win-signer-build.assert.sig'):
            subprocess.check_call(['git', 'add', args.version+'-win-signed/'+args.signer])
            commit = True
        if os.path.isfile(args.version+'-osx-signed/'+args.signer+'/rpicoin-dmg-signer-build.assert.sig'):
...
def verify():
    ...
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-linux', '../rpicore/contrib/gitian-descriptors/gitian-linux.yml']):
        print('Verifying v'+args.version+' Linux FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' Windows\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-unsigned', '../rpicore/contrib/gitian-descriptors/gitian-win.yml']):
        print('Verifying v'+args.version+' Windows FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-unsigned', '../rpicore/contrib/gitian-descriptors/gitian-osx.yml']):
        print('Verifying v'+args.version+' MacOS FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' Signed Windows\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-signed', '../rpicore/contrib/gitian-descriptors/gitian-win-signer.yml']):
        print('Verifying v'+args.version+' Signed Windows FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' Signed MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-signed', '../rpicore/contrib/gitian-descriptors/gitian-osx-signer.yml']):
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/rpicoin/rpicore', help='Specify the URL of the repository. Default is %(default)s')
    ...
    if args.pull:
        ...
        if not os.path.isdir('../gitian-builder/inputs/rpicoin'):
            os.makedirs('../gitian-builder/inputs/rpicoin')
        os.chdir('../gitian-builder/inputs/rpicoin')
