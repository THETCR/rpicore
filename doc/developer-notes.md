...
- [Development guidelines](#development-guidelines)
    - [General RPICOIN Core](#general-rpicoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
RPICOIN Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play RPIs (tRPI)" on the test network, if you
...
RPICOIN Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/rpicoin/rpicore/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_rpicoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_rpicoin --log_level=test_suite
$ valgrind -v --leak-check=full src/rpicoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_rpicoin.coverage/index.html`.
```
...
- BitcoinMiner : Generates RPIs (if wallet is enabled).
...
However, in open source software such as RPICOIN Core, where everyone uses
...
pay attention to for reviewers of RPICOIN Core code.

General RPICOIN Core
----------------------
...
- Avoid using locale dependent functions if possible. You can use the provided
  ...
  - These functions are known to be locale dependent:
    ...
    `iswpunct`, `isrpiace`, `iswupper`, `iswxdigit`, `isxdigit`, `mblen`,
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: RPICOIN Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against RPICOIN Core so that they can be integrated
...
file descriptors it will cause problems with RPICOIN's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:rpicoin/rpicore.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `rpicoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `rpicoin-cli` and the GUI debug console use this table to determine how to
