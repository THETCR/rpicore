...
*Setup instructions for a gitian build of RPICOIN Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the RPICOIN
...
to the RPICOIN GitHub release page.
...
- [Building RPICOIN Core](#building-rpicoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/rpicoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/rpicoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/rpicoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/rpicoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/rpicoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/rpicoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/rpicoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@rpicoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/rpicoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/rpicoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/rpicoin-osx-*-build.assert
```
